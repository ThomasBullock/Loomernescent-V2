...
test("valid registration → redirected to home, welcome flash shown", async ({ page }) => {
  const registerPage = new RegisterPage(page);
  const layout = new LayoutPage(page);

  await registerPage.goto();
  await registerPage.fill("New User", `new-${Date.now()}@example.test`, "Password1!", "Password1!");
...
test("password mismatch → re-renders register with error", async ({ page }) => {
  ...
  await registerPage.goto();
  await registerPage.fill("Test", `mismatch-${Date.now()}@example.test`, "Password1!", "different");
  await registerPage.submit();
...
test("password too short → re-renders register with error", async ({ page }) => {
  ...
  await registerPage.goto();
  await registerPage.fill("Test", `short-${Date.now()}@example.test`, "short", "short");
  await registerPage.submit();
...
test("honeypot field filled → 400 response", async ({ page }) => {
  ...
  await page.locator('input[name="stratosphere"]').evaluate((el: HTMLInputElement) => {
    el.value = "bot-value";
  });
