...
function makeAlbumsResponse(albums: Array<{ id: string; name: string; spotify: string }>) {
  return {
    ok: true,
    json: () =>
      Promise.resolve({
...
describe("SpotifyService", () => {
  ...
  describe("getAccessToken", () => {
    it("fetches a token using Basic auth and client_credentials grant", async () => {
      ...
      const expectedCredentials = Buffer.from(`${CLIENT_ID}:${CLIENT_SECRET}`).toString("base64");
    ...
    it("throws when token endpoint returns an error", async () => {
      ...
      await expect(service.getAccessToken()).rejects.toThrow("Spotify token request failed: 401");
    });
  });
  ...
  describe("getAlbumTracks", () => {
    ...
    it("returns array of track names", async () => {
      fetchSpy.mockResolvedValueOnce(makeTracksResponse(["Alison", "Machine Gun", "Sing"]));
